...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // Calculate how full the leafNodes should be
        ...
        // Split the leafNode
        DataBox key = keys.remove(maxSize);
        RecordId rid = rids.remove(maxSize);
        ...
        return Optional.of(new Pair<>(key, rightNodePageNum));
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        long rightSiblingByte = buf.getLong();
        Optional<Long> rightSibling;

        if (rightSiblingByte == -1L) {
            rightSibling = Optional.empty();
        } else {
            rightSibling = Optional.of(rightSiblingByte);
        }
